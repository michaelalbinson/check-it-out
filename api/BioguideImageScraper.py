...
def get_wiki_photo(first, middle, last):
    if middle is None or len(middle) < 3:
        ...
        get_url = 'https://en.wikipedia.org/w/api.php?action=query&titles=' + first + '_' + middle + '_' + last + \
                  '&prop=images&imlimit=20&format=json'
    ...
    if "-1" not in wiki_json['query']['pages']:
        file = get_most_relevant_file_name(wiki_json['query']['pages'], first, middle, last)
    ...
    return 'https://upload.wikimedia.org/wikipedia/commons/thumb/5/5b/Neil_Abercrombie.jpg/480px-Neil_Abercrombie.jpg'
    # our last ditch attempt has failed, I hate you internet, I quit


def get_most_relevant_file_name(pages, first, middle, last):
    if middle is None:
        middle = ''
    relevant_title = ''
    best_relevance = 0
    ...
    for key in pages:
        ...
        for image in images:
            ...
            if (
                    first + ' ' + last) in current_title:  # give highest precedence to something that has both names in order
                current_relevance += 2

            if (first + ' ' + middle + ' ' + last) in current_title:
                current_relevance += 3
